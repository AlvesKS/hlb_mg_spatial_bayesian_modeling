...
# Función que a partir de una formula de vuelve el mejor modelo según el dic ##########
########################### (inla stack) ##############################################
#######################################################################################
...
###### * n: número de modelos que queremos que muestre a la salida ####################
###### * family: familia a la que pertenece la variable repuesta, por defecto binomial#
###### * ...: argumentos de la función inla ###########################################
#######################################################################################
...
{  
  ...
  modelos_waic<-data.frame(f.list[order(waic)[1:n]], dic[order(waic)[1:n]], waic[order(waic)[1:n]], LCPO[order(waic)[1:n]], LCPOfailure[order(waic)[1:n]])
  colnames(modelos_waic)<-c("Modelos", "Dic", "Waic", "LCPO", "LCPO-failure")
  
  
  #mostramos los modelos ordenados SEGÚN EL LCPO
  modelos_lcpo<-data.frame(f.list[order(LCPO)[1:n]], dic[order(LCPO)[1:n]], waic[order(LCPO)[1:n]], LCPO[order(LCPO)[1:n]], LCPOfailure[order(LCPO)[1:n]])
  colnames(modelos_lcpo)<-c("Modelos", "Dic", "Waic", "LCPO", "LCPO-failure")
  

  modelos<-list(modelos_dic, modelos_waic, modelos_lcpo)
  names(modelos)<-c("Modelos segun dic", "Modelos segun waic", "Modelos segun lcpo")
  modelos
